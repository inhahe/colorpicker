...
- **Palette** button — toggles the current palette as a 3D trace (highlighted while shown; click again to hide)
...
- **Load Image** / **Image** buttons — **Load Image** loads (or replaces) an image and plots only its colors in the model; the **Image** toggle then shows/hides that image distribution (highlighted while on, greyed out until an image is loaded), so you can switch back to the full color model without unloading. The image persists across space changes; the × on the image-name indicator unloads it entirely.
